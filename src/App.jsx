...
function emptyCard() {
  return { baths: makeBathSlots(), grooms: makeGroomSlots(), archivedCycles: [] };
}

function emptyPet(name) {
  return {
  ...
  return (
    <div
      style={{
        ...
        background: isFree
          ? filled
            ? `linear-gradient(135deg,${FREE_COLOR}cc,${FREE_COLOR}88)`
            : "#1a3a3a"
          : filled
            ? "#0a2e2e"
            : "#0a1a1a",
    ...
    >
      ...
      >
        {filled ? (
          ...
          <span style={{ fontSize: 11, fontWeight: 900, color: FREE_COLOR }}>&#9733;</span>
        ) : (
      ...
      {filled ? (
        <span style={{ fontSize: 11, color: isFree ? "#ffffff99" : "#5de7edbb", fontFamily: "monospace", fontWeight: 700 }}>
          {slot.date}
        </span>
      ) : (
        <input
      ...
      {!filled ? (
        <button
          onClick={punch}
          style={{
            background: isFree ? FREE_COLOR : "#0cc0df",
            border: "none",
            borderRadius: 6,
            color: "#000",
            fontSize: 9,
...
function GroomSlot({ slot, idx, onChange, freeType }) {
  ...
  return (
    ...
    >
      ...
      >
        {filled ? (
          <span style={{ fontSize: 13, color: isFree ? "#fff" : tc, fontWeight: 900 }}>&#10003;</span>
        ) : isFree ? (
          <span style={{ fontSize: 11, fontWeight: 900, color: FREE_COLOR }}>&#9733;</span>
        ) : (
          <span style={{ fontSize: 11, color: "#6ababa", fontWeight: 800, fontFamily: "monospace" }}>{idx + 1}</span>
      ...
      <span
        style={{
          ...
          flex: 1,
          minWidth: 0,
          color: isFree ? (filled ? "#fff" : FREE_COLOR) : filled ? tc : "#8ad8d8",
          fontFamily: "monospace",
          letterSpacing: 0.5,
          textTransform: "uppercase",
          whiteSpace: "nowrap",
      ...
      >
        ...
        {filled && slot.type && !isFree && (
          <span style={{ marginLeft: 5, fontSize: 9, background: tc + "33", color: tc, borderRadius: 4, padding: "1px 5px", fontWeight: 800 }}>
            {slot.type}
          </span>
        )}
      </span>
      ...
      {!filled && !isFree && (
        <select
          ...
          style={{
            background: "#111",
            border: "2px solid " + TYPE_COLORS[selType],
            borderRadius: 6,
      ...
      {filled ? (
        <span style={{ fontSize: 10, color: isFree ? "#ffffff99" : tc + "bb", fontFamily: "monospace", flexShrink: 0, fontWeight: 700 }}>
          {slot.date}
        </span>
      ) : (
        <input
      ...
      {!filled ? (
        ...
        <button
          ...
          style={{ background: "transparent", border: "none", color: isFree ? "#ffffff55" : "#5a9a9a", cursor: "pointer", fontSize: 12, padding: "0 2px", flexShrink: 0 }}
        >
          &#10005;
        </button>
      )}
...
function PetCard({ pet, onUpdate, onRemove, isOnly }) {
  ...
  const punchB = (i, v) => {
    const b = pet.card.baths.map((s, j) => (j === i ? { ...s, ...v } : s));
    onUpdate({ ...pet, card: { ...pet.card, baths: b } });
  };

  const punchG = (i, v) => {
    const g = pet.card.grooms.map((s, j) => (j === i ? { ...s, ...v } : s));
    onUpdate({ ...pet, card: { ...pet.card, grooms: g } });
  };
    ...
    <div style={{ background: "#0e1a1a", border: "1px solid " + (done ? "#4edee4" : "#1a3333"), borderRadius: 16, overflow: "hidden", marginBottom: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 6, padding: "10px 12px", background: "linear-gradient(135deg,#0a2020,#061818)", borderBottom: "1px solid #1a3333" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ width: 30, height: 30, borderRadius: "50%", background: "#0cc0df22", border: "1px solid #0cc0df44", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
            &#128062;
          </span>
          ...
          {editName ? (
            <input
              ...
              onBlur={() => {
                onUpdate({ ...pet, name: tmpName });
                setEditName(false);
              }}
              onKeyDown={(e) => e.key === "Enter" && e.target.blur()}
              placeholder="Pet name..."
              style={{ background: "#061414", border: "1px solid #0cc0df", borderRadius: 6, color: "#fff", fontSize: 15, fontWeight: 700, padding: "3px 10px", outline: "none", fontFamily: "system-ui,sans-serif" }}
            />
          ) : (
            <div>
              <span onClick={() => setEditName(true)} style={{ fontSize: 16, fontWeight: 800, color: "#fff", cursor: "pointer", fontFamily: "system-ui,sans-serif", borderBottom: "1px dashed #2a5555" }}>
                {pet.name || "TAP TO NAME PET"}
              </span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {arcs.length > 0 && (
            <button onClick={() => setShowArc(!showArc)} style={{ background: "transparent", border: "1px solid #2a5555", color: "#6ababa", borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontSize: 10, fontFamily: "monospace", letterSpacing: 1 }}>
              {arcs.length} CYCLE{arcs.length > 1 ? "S" : ""}
            </button>
          )}

          {done && (
            <button onClick={reset} style={{ background: "linear-gradient(135deg,#0cc0df,#4edee4)", border: "none", color: "#000", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 10, fontWeight: 800, fontFamily: "monospace", letterSpacing: 1 }}>
              NEW CYCLE
            </button>
          )}
          ...
          {!isOnly && (
            <button onClick={onRemove} style={{ background: "transparent", border: "none", color: "#4a8a8a", cursor: "pointer", fontSize: 15 }}>
              &#10005;
            </button>
          )}
        </div>
      ...
      <div style={{ padding: "8px 16px 0", display: "flex", gap: 12 }}>
        {[
          { label: "Bath", pct: bPct, free: bFr },
          { label: "Groom", pct: gPct, free: gFr },
        ].map(({ label, pct, free }) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
        <div style={{ padding: "10px 12px", borderRight: "1px solid #0f2626" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {pet.card.baths.map((slot, i) => (
              <BathSlot key={i} slot={slot} idx={i} onChange={(v) => punchB(i, v)} />
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: "10px 12px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {pet.card.grooms.map((slot, i) => (
              <GroomSlot key={i} slot={slot} idx={i} onChange={(v) => punchG(i, v)} freeType={fgt} />
            ))}
          </div>
        </div>
      </div>
...
function ClientDetail({ client, onUpdate, onBack, onDelete }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: "0 auto", minWidth: 0 }}>
      <button onClick={onBack} style={{ background: "transparent", border: "none", color: "#0cc0df", cursor: "pointer", fontSize: 12, marginBottom: 18, display: "flex", alignItems: "center", gap: 6, fontFamily: "monospace", letterSpacing: 2, padding: 0 }}>
        &#8592; ALL CLIENTS
      </button>
      ...
      <div style={{ background: "linear-gradient(135deg,#071a1a,#040f0f)", border: "1px solid #1a3333", borderRadius: 16, padding: "16px 20px", marginBottom: 18 }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {!confirmDel ? (
              <button onClick={() => setConfirmDel(true)} style={{ background: "#1a0808", border: "1px solid #3a1515", color: "#c05050", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 11, fontFamily: "monospace", letterSpacing: 1 }}>
                ARCHIVE
              </button>
            ) : (
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#1a0808", border: "1px solid #3a1515", borderRadius: 8, padding: "6px 12px" }}>
                ...
                <button onClick={() => { if (delInput === "DELETE") onDelete(); }} disabled={delInput !== "DELETE"} style={{ background: delInput === "DELETE" ? "#c05050" : "#2a1010", border: "none", borderRadius: 6, color: "#fff", fontSize: 11, fontWeight: 800, padding: "3px 8px", cursor: delInput === "DELETE" ? "pointer" : "not-allowed", fontFamily: "monospace" }}>
                  &#10003;
                </button>
                <button onClick={() => { setConfirmDel(false); setDelInput(""); }} style={{ background: "transparent", border: "none", color: "#555", cursor: "pointer", fontSize: 14 }}>
                  &#10005;
                </button>
              </div>
            )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <button onClick={addPet} style={{ background: "#061a1a", border: "1px solid #0cc0df44", color: "#4edee4", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontFamily: "monospace", letterSpacing: 1 }}>
          + ADD PET
        </button>
      </div>
      ...
      {c.pets.map((pet) => (
        <PetCard
          ...
          pet={pet}
          onUpdate={(p) => {
            const u = { ...c, pets: c.pets.map((x) => (x.id === p.id ? p : x)) };
            setC(u);
            onUpdate(u);
          }}
          onRemove={() => removePet(pet.id)}
          isOnly={c.pets.length === 1}
        />
      ))}
    </div>
  );
...
export default function App() {
  ...
  const filtered = active.filter(
    (c) =>
      ...
      (fmt(c.name).includes(fmt(search)) ||
        fmt(c.phone).includes(fmt(search)) ||
  ...
  const letters = ["ALL", ..."ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("")].filter((l) => l === "ALL" || active.some((c) => (c.name || "").toUpperCase().startsWith(l)));
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f0f", color: "#fff" }}>
      <div style={{ background: "#000", borderBottom: "2px solid #0cc0df", padding: "0 14px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, position: "sticky", top: 0, zIndex: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 19, fontWeight: 900, letterSpacing: 1, fontFamily: "monospace", background: "linear-gradient(90deg,#4edee4,#0cc0df)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", lineHeight: 1.1 }}>
              SUDS N&#39; BONES
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={exportCSV} style={{ background: "transparent", border: "1px solid #0cc0df44", color: "#4edee4", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 11, fontFamily: "monospace", letterSpacing: 1, fontWeight: 800, whiteSpace: "nowrap" }}>
            &#11015; EXPORT
          </button>
        ...
        {selected ? (
          ...
          <>
            <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              <button onClick={() => setAdding(true)} style={{ background: "linear-gradient(135deg,#0cc0df,#4edee4)", border: "none", color: "#000", borderRadius: 10, padding: "0 18px", cursor: "pointer", fontWeight: 900, fontSize: 14, letterSpacing: 1, fontFamily: "monospace", whiteSpace: "nowrap" }}>
                + NEW CLIENT
              </button>
            </div>
            ...
            {adding && (
              <div style={{ background: "#060f0f", border: "1px solid #0cc0df33", borderRadius: 14, padding: "14px 18px", marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                ...
                <button onClick={addClient} style={{ background: "linear-gradient(135deg,#0cc0df,#4edee4)", border: "none", color: "#000", borderRadius: 10, padding: "9px 18px", cursor: "pointer", fontWeight: 900, fontFamily: "monospace", letterSpacing: 1 }}>
                  ADD &#8594;
                </button>

                <button onClick={() => setAdding(false)} style={{ background: "transparent", border: "1px solid #2a5555", color: "#5a9a9a", borderRadius: 10, padding: "9px 14px", cursor: "pointer", fontSize: 13 }}>
                  &#10005;
                </button>
              </div>
            )}
              ...
              {letters.map((l) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    padding: l === "ALL" ? "4px 10px" : "4px 7px",
                    cursor: "pointer",
                    fontSize: l === "ALL" ? 10 : 12,
                    fontWeight: 800,
                    fontFamily: "monospace",
                    minWidth: l === "ALL" ? 40 : 28,
                ...
                {pageClients.map((c) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.borderColor = "#0cc0df88";
                        e.currentTarget.style.background = "#0a1818";
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.borderColor = hasFree ? "#0cc0df55" : "#0f2020";
                        e.currentTarget.style.background = "#070f0f";
                      }}
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 13, color: "#5a9a9a", fontFamily: "monospace", marginTop: 2 }}>
                          {c.phone || "No phone"} &#183; {(c.pets || []).length} pet{(c.pets || []).length !== 1 ? "s" : ""}
                          {(c.pets || []).some((p) => p.name) ? ": " + c.pets.map((p) => p.name || "?").join(", ") : ""}
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
                <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} style={{ background: page === 0 ? "#060f0f" : "#061a1a", border: "1px solid #1a3333", color: page === 0 ? "#2a4444" : "#4edee4", borderRadius: 8, padding: "8px 16px", cursor: page === 0 ? "not-allowed" : "pointer", fontSize: 12, fontFamily: "monospace", fontWeight: 800, letterSpacing: 1 }}>
                  &#8592; PREV
                </button>

                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
                  {Array.from({ length: totalPages }, (_, i) => (
                    <button key={i} onClick={() => setPage(i)} style={{ background: page === i ? "linear-gradient(135deg,#0cc0df,#4edee4)" : "#060f0f", border: "1px solid " + (page === i ? "#0cc0df" : "#1a3333"), color: page === i ? "#000" : "#6ababa", borderRadius: 6, padding: "6px 10px", cursor: "pointer", fontSize: 11, fontFamily: "monospace", fontWeight: 800, minWidth: 32 }}>
                      {i + 1}
                    </button>
                  ))}
                </div>

                <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} style={{ background: page === totalPages - 1 ? "#060f0f" : "#061a1a", border: "1px solid #1a3333", color: page === totalPages - 1 ? "#2a4444" : "#4edee4", borderRadius: 8, padding: "8px 16px", cursor: page === totalPages - 1 ? "not-allowed" : "pointer", fontSize: 12, fontFamily: "monospace", fontWeight: 800, letterSpacing: 1 }}>
                  NEXT &#8594;
                </button>
              </div>
            )}
            ...
            {archived.length > 0 && !search && activeLetter === "ALL" && (
              <div style={{ marginTop: 24 }}>
                <div style={{ fontSize: 10, letterSpacing: 2, color: "#3a5555", fontFamily: "monospace", marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {archived.map((c) => (
                    <div key={c.id} style={{ background: "#060c0c", border: "1px solid #0f1e1e", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, opacity: 0.6 }}>
                      ...
                      <button onClick={() => persist(clients.map((x) => (x.id === c.id ? { ...x, deleted: false, deletedAt: null } : x)))} style={{ background: "#061a1a", border: "1px solid #0cc0df44", color: "#4edee4", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 10, fontFamily: "monospace", letterSpacing: 1, flexShrink: 0 }}>
                        RESTORE
                      </button>
                    </div>
                  ))}
